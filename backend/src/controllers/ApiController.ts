...
export const index = async (req: Request, res: Response): Promise<Response> => {
  ...
  const convertBinToPdfIfNeeded = (
  ...
  ): { media: Express.Multer.File; filePath: string } => {
    const publicFolder = path.resolve(__dirname, "..", "..", "public");
    const originalPath = path.join(publicFolder, `company${companyId}`, media.filename);

    const ext = path.extname(media.originalname || media.filename || "").toLowerCase();
    ...
    if (isBinExt || isBinMime) {
      ...
      const newPath = path.join(publicFolder, `company${companyId}`, newFilename);
      ...
      try {
        await Promise.all(
          medias.map(async (media: Express.Multer.File) => {
            ...
            const publicFolder = path.resolve(__dirname, "..", "..", "public");
            const filePath = path.join(publicFolder, `company${companyId}`, adjustedMedia.filename);
            const fileExists = fs.existsSync(filePath);
