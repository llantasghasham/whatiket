...
import ListPublicPlansSimpleService from "../services/PlanService/ListPublicPlansSimpleService";
...
export const listPublicSimple = async (_req: Request, res: Response): Promise<Response> => {
  const plans = await ListPublicPlansSimpleService();
  return res.status(200).json(plans);
};
