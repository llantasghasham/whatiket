...
const ExecuteAutomationsJob_1 = __importStar(require("./services/AutomationServices/ExecuteAutomationsJob"));
const DispatchSchedulerService_1 = __importDefault(require("./services/ScheduledDispatcherService/DispatchSchedulerService"));
const DispatchProcessorService_1 = __importDefault(require("./services/ScheduledDispatcherService/DispatchProcessorService"));
...
async function startQueueProcess() {
    ...
    const automationJob = new CronJob('*/2 * * * *', async () => {
        try {
            await (0, ExecuteAutomationsJob_1.default)();
        }
        catch (error) {
            logger_1.default.error(`[Automation Job] Erro: ${error}`);
        }
    });
    automationJob.start();
    ...
    const birthdayJob = new CronJob('0 9 * * *', async () => {
        try {
            await (0, ExecuteAutomationsJob_1.runBirthdayAutomationJob)();
        }
        catch (error) {
            logger_1.default.error(`[Automation Birthday Job] Erro: ${error}`);
        }
    });
    birthdayJob.start();
    ...
    const kanbanJob = new CronJob('*/10 * * * *', async () => {
        try {
            await (0, ExecuteAutomationsJob_1.runKanbanAutomationJob)();
        }
        catch (error) {
            logger_1.default.error(`[Automation Kanban Job] Erro: ${error}`);
        }
    });
    kanbanJob.start();
    ...
    const noResponseJob = new CronJob('*/15 * * * *', async () => {
        try {
            await (0, ExecuteAutomationsJob_1.runNoResponseAutomationJob)();
        }
        catch (error) {
            logger_1.default.error(`[Automation NoResponse Job] Erro: ${error}`);
        }
    });
    noResponseJob.start();
    ...
    const dispatchJob = new CronJob('*/5 * * * *', async () => {
        try {
            await (0, DispatchSchedulerService_1.default)();
        }
        catch (error) {
            logger_1.default.error(`[Scheduled Dispatch Job] Erro: ${error}`);
        }
    });
    dispatchJob.start();
    ...
    const cleanContactsJob = new CronJob('0 7,19 * * *', async () => {
        try {
            ...
            await (0, CleanLidContactsRunner_1.runCleanLidContacts)();
            logger_1.default.info("[cleanLidContacts Job] Execução concluída");
        }
        catch (error) {
            logger_1.default.error(`[cleanLidContacts Job] Erro: ${error}`);
        }
    });
