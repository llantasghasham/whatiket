...
const ALLOWED_USER_TYPES = ["admin", "manager", "attendant", "professional"];
const CreateUserService = async ({ email, password, name, queueIds = [], serviceIds = [], companyId, profile = "admin", startWork, endWork, whatsappId, allTicket, defaultTheme, defaultMenu, allowGroup, allHistoric, allUserChat, userClosePendingTicket, showDashboard, defaultTicketsManagerWidth = 550, allowRealTime, allowConnections, userType = "attendant", workDays = "1,2,3,4,5", lunchStart, lunchEnd }) => {
    ...
    const schema = Yup.object().shape({
        ...
        userType: Yup.mixed().oneOf([...ALLOWED_USER_TYPES])
    });
    try {
