...
const UpdateUserService = async ({ userData, userId, companyId, requestUserId }) => {
    ...
    const userCompany = await Company_1.default.findByPk(user.companyId);
    ...
    const { email, password, profile, name, queueIds = [], serviceIds = [], startWork, endWork, farewellMessage, whatsappId, allTicket, defaultTheme, defaultMenu, allowGroup, allHistoric, allUserChat, userClosePendingTicket, showDashboard, allowConnections, defaultTicketsManagerWidth = 550, allowRealTime, profileImage, userType, workDays, lunchStart, lunchEnd } = userData;
