...
const ShowPromptService_1 = __importDefault(require("../PromptServices/ShowPromptService"));
...
const ContactPhoneService_1 = require("../ContactServices/ContactPhoneService");
...
const WaitQuestionService_1 = __importDefault(require("../FlowBuilderService/WaitQuestionService"));
...
const getBodyButton = (msg) => {
    try {
        ...
        if (msg?.messageType === "viewOnceMessage" ||
            msg?.message?.viewOnceMessage?.message?.interactiveMessage) {
            let bodyMessage = "";
            const buttons = msg?.message?.viewOnceMessage?.message?.interactiveMessage
        ...
        if (msg?.messageType === "interactiveMessage" ||
            ...
            const bodyTextWithPix = Array.isArray(buttons) &&
        ...
        if (msg?.messageType === "viewOnceMessage" ||
            ...
            const bodyTextWithPix = msg?.message?.viewOnceMessage?.message?.interactiveMessage?.header
                ?.title;
...
const getBodyMessage = (msg) => {
    try {
        ...
        const types = {
            ...
            adMetaPreview: msgAdMetaPreview(msg.message?.extendedTextMessage?.contextInfo?.externalAdReply
                ?.thumbnail, msg.message?.extendedTextMessage?.contextInfo?.externalAdReply?.title, msg.message?.extendedTextMessage?.contextInfo?.externalAdReply?.body, msg.message?.extendedTextMessage?.contextInfo?.externalAdReply
                ?.sourceUrl, msg.message?.extendedTextMessage?.text),
...
const getSenderMessage = async (msg, wbot) => {
...
const getContactMessage = async (msg, wbot, senderPn) => {
    ...
    if (isNewsletter) {
        logger_1.default.info(`[newsletter] Ignorando mensagem de newsletter: ${msg.key.remoteJid}`);
        return null;
    }
    const baseNumber = (0, normalizeContactNumber_1.resolveContactNumber)({
    ...
    const senderDigits = (0, normalizeContactNumber_1.resolveContactNumber)({
...
const downloadMedia = async (msg, isImported = null, wbot, ticket) => {
    ...
    if (wbot &&
        ...
        const sendMsg = await wbot.sendMessage(`${ticket.contact.number}@${"s.whatsapp.net"}`, fileLimitMessage);
...
const verifyContact = async (msgContact, wbot, companyId, msg) => {
    logger_1.default.info("=== VERIFY CONTACT START ===");
    logger_1.default.info("msgContact received:", {
        id: msgContact.id,
    ...
    const number = (0, normalizeContactNumber_1.resolveContactNumber)({
    ...
    if (!isGroup && wbot) {
        try {
            ...
            logger_1.default.info("Got profile picture URL:", profilePicUrl);
        }
        catch (err) {
            logger_1.default.debug("Could not get profile picture:", err?.message);
            profilePicUrl = "";
        }
    }
...
const verifyMediaMessage = async (msg, ticket, contact, ticketTraking, isForwarded = false, isPrivate = false, wbot, isMessageImported = false) => {
...
const verifyMessage = async (msg, ticket, contact, ticketTraking, isPrivate, isForwarded = false, isMessageImported = false, fromAgent = false, userId) => {
            ...
            if (choosenQueue.chatbots.length > 0 && !ticket.isGroup) {
                ...
                const debouncedSentMessage = (0, Debounce_1.debounce)(async () => {
                    try {
                        ...
                        const newMsg = (0, baileys_1.generateWAMessageFromContent)(jid, interactiveMsg, {
                            userJid: botNumber
                        });
...
const flowbuilderIntegration = async (msg, wbot, companyId, queueIntegration, ticket, contact, isFirstMsg, isTranfered) => {
    ...
    if (ticket.waitingQuestion && !msg.key.fromMe) {
        ...
        try {
            const response = await WaitQuestionService_1.default.processResponse(ticket.id, body);
            if (response) {
                ...
                switch (response.action) {
                    ...
                    case "transfer":
                        ...
                        if (flow && flow.flow) {
                            ...
                            const targetConnection = connections.find(conn => conn.source === response.nodeId &&
                                conn.sourceHandle === response.option);
                            if (targetConnection) {
                                await (0, ActionsWebhookService_1.ActionsWebhookService)(ticket.whatsappId, parseInt(ticket.flowStopped), ticket.companyId, flow.flow["nodes"] || [], connections, targetConnection.target, ticket.dataWebhook, "", "", body, ticket.id, {
                                    number: contact.number,
                                    name: contact.name,
                                    email: contact.email
                                }, msg);
                    ...
                    default:
                        ...
                        const flowContinue = await FlowBuilder_1.FlowBuilderModel.findOne({
                            where: { id: ticket.flowStopped }
                        });
                        ...
                        if (flowContinue && flowContinue.flow) {
                            ...
                            const targetConnectionContinue = connectionsContinue.find(conn => conn.source === response.nodeId &&
                                conn.sourceHandle === response.option);
                            if (targetConnectionContinue) {
                                console.log(`[WaitQuestion] Conexão encontrada! Target: ${targetConnectionContinue.target}`);
                                await (0, ActionsWebhookService_1.ActionsWebhookService)(ticket.whatsappId, parseInt(ticket.flowStopped), ticket.companyId, flowContinue.flow["nodes"] || [], connectionsContinue, targetConnectionContinue.target, ticket.dataWebhook, "", "", body, ticket.id, {
                                    number: contact.number,
                                    name: contact.name,
                                    email: contact.email
                                }, msg);
    ...
    */
    ...
    const campaignMatchesBody = (campaign) => {
    ...
    if (!isFirstMsg &&
        ...
        if (flow) {
            ...
            // });
            await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, whatsapp.flowIdWelcome, ticket.companyId, nodes, connections, flow.flow["nodes"][0].id, null, "", "", null, ticket.id, mountDataContact, msg || null);
        }
    }
    ...
    if (listPhrase.filter(item => campaignMatchesBody(item)).length === 0 &&
        ...
        if (flow) {
            ...
            await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, whatsapp.flowIdNotPhrase, ticket.companyId, nodes, connections, flow.flow["nodes"][0].id, null, "", "", null, ticket.id, mountDataContact, msg || null);
        }
    }
    // Campaign fluxo
    ...
    if (matchingCampaigns.length !== 0) {
        ...
        //   console.log(`Mensagem do worker: ${message}`);
        // });
        await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, flowDispar.flowId, ticket.companyId, nodes, connections, flow.flow["nodes"][0].id, null, "", "", null, ticket.id, mountDataContact, msg || null);
        return;
    }
    if (ticket.flowWebhook) {
        ...
        if (ticket.hashFlowId) {
            webhook = await Webhook_1.WebhookModel.findOne({
                where: {
        ...
        if (webhook && webhook.config["details"]) {
            ...
            //   console.log(`Mensagem do worker: ${message}`);
            // });
            await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, webhook.config["details"].idFlow, ticket.companyId, nodes, connections, ticket.lastFlowId, ticket.dataWebhook, webhook.config["details"], ticket.hashFlowId, body, ticket.id, null, msg || null);
        }
        else {
            ...
            if (ticket.lastFlowId === null || ticket.lastFlowId === undefined) {
                ...
                const startConnection = connections.find((c) => c.source === startNode.id);
                if (!startConnection) {
                ...
                await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, parseInt(ticket.flowStopped), ticket.companyId, nodes, connections, startConnection.target, null, "", "", body, ticket.id, {
                    number: contact.number,
                    name: contact.name,
                    email: contact.email
                }, msg || null);
            ...
            //   console.log(`Mensagem do worker: ${message}`);
            // });
            await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, parseInt(ticket.flowStopped), ticket.companyId, nodes, connections, ticket.lastFlowId, null, "", "", body, ticket.id, mountDataContact, msg || null);
        }
    }
};
const handleMessageIntegration = async (msg, wbot, companyId, queueIntegration, ticket, isMenu, whatsapp, contact, isFirstMsg) => {
    ...
    if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
        ...
        if (!isMenu) {
            ...
            //   companyId
            // );
            await flowbuilderIntegration(msg, wbot, companyId, undefined, //integrations,
...
const flowBuilderQueue = async (ticket, msg, wbot, whatsapp, companyId, contact, isFirstMsg) => {
    ...
    if (ticket.waitingQuestion && !msg.key.fromMe) {
        ...
        try {
            ...
            if (response) {
                ...
                const flowWQ = await FlowBuilder_1.FlowBuilderModel.findOne({
                    where: { id: ticket.flowStopped }
                });
                ...
                if (flowWQ && flowWQ.flow) {
                    ...
                    const targetConnection = connectionsWQ.find(conn => conn.source === response.nodeId &&
                    ...
                    if (targetConnection) {
                        ...
                        await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, parseInt(ticket.flowStopped), ticket.companyId, flowWQ.flow["nodes"] || [], connectionsWQ, targetConnection.target, ticket.dataWebhook, "", "", body, ticket.id, mountDataContact, msg);
    ...
    if (!ticket.lastFlowId) {
        ...
        // Executar o primeiro nó
        await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, parseInt(ticket.flowStopped), ticket.companyId, nodes, connections, startConnection.target, null, "", "", body, ticket.id, mountDataContact, msg || null);
        return;
    }
...
const handleMessage = async (msg, wbot, companyId, isImported = false) => {
    ...
    if (msg.key.remoteJid.includes("newsletter")) {
        logger_1.default.info(`[newsletter] Ignorando mensagem de newsletter em handleMessage: ${msg.key.remoteJid}`);
        return;
    }
    if (!isValidMsg(msg)) {
    ...
    try {
        ...
        if (msg.key.fromMe) {
            const isAiMessage = await cache_1.default.get(aiMessageKey);
            if (isAiMessage) {
                await cache_1.default.del(aiMessageKey);
            }
            else {
                const pauseUntil = (0, date_fns_1.add)(new Date(), { minutes: 30 }).getTime();
                await cache_1.default.set(botPauseKey, pauseUntil.toString(), "EX", 60 * 60);
                if (ticket.isBot) {
        ...
        if (!useLGPD) {
            ...
            if (hasMedia) {
                ...
                persistedMessage = mediaSent;
            }
            else {
                console.log("log... 3396");
                persistedMessage = await (0, exports.verifyMessage)(msg, ticket, contact, ticketTraking, false, isMsgForwarded);
            }
        }
        try {
            if (!msg.key.fromMe) {
        ...
        if (!(0, lodash_1.isNil)(flow) &&
            ...
            await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, parseInt(ticket.flowStopped), ticket.companyId, nodes, connections, ticket.lastFlowId, ticket.dataWebhook, "", "", body, ticket.id, mountDataContact, msg || null);
            return;
        }
        if (!(0, lodash_1.isNil)(flow) && isQuestion && !msg.key.fromMe) {
            ...
            if (body) {
                ...
                const answerKey = questionConfig.answerKey || `question_${nodeSelected.id}`;
                ...
                if (updatedWebhookData.questionState?.nodeId === nodeSelected.id &&
                    updatedWebhookData.questionState?.awaiting) {
                    delete updatedWebhookData.questionState;
                }
                const outgoingConnection = connections.find(connection => connection.source === nodeSelected.id);
                ...
                await (0, ActionsWebhookService_1.ActionsWebhookService)(whatsapp.id, parseInt(ticket.flowStopped), ticket.companyId, nodes, connections, nextFlowId, updatedWebhookData, "", "", "", ticket.id, mountDataContact, msg || null);
            }
            return;
        }
        // IA do FlowBuilder: só responde se ticket NÃO tiver usuário vinculado
        if (ticket.isBot && isOpenai && !(0, lodash_1.isNil)(flow) && !ticket.queue && !ticket.userId) {
            ...
            const cfg = nodeSelected.data.typebotIntegration || {};
            ...
            if (cfg.iaMode === "system" && !Number.isNaN(promptIdNumber)) {
                const prompt = await (0, ShowPromptService_1.default)({
                ...
                let { name, prompt, voice, voiceKey, voiceRegion, maxTokens, temperature, apiKey, queueId, maxMessages } = cfg;
                openAiSettings = {
                    name,
                    prompt,
                    voice,
        ...
        if (!isGroup &&
            ...
            if (prompt) {
                try {
                    ...
                    prompt.toolsEnabled = toolsEnabled;
                }
                catch (error) {
        ...
        if (ticket.typebotStatus &&
            ...
            try {
                ...
                if (status === 404 && data?.message === "Session not found.") {
            ...
            //check motives
            console.log("!isNil(ticket.typebotSessionId): ", !(0, lodash_1.isNil)(ticket.typebotSessionId));
            console.log("ticket.typebotStatus: ", ticket.typebotStatus);
            console.log("!msg.key.fromMe: ", !msg.key.fromMe);
            console.log("!isNil(ticket.typebotSessionTime): ", !(0, lodash_1.isNil)(ticket.typebotSessionTime));
    ...
    wbot.ev.on("messages.upsert", async (messageUpsert) => {
        ...
        messages.forEach(async (message) => {
            ...
            if (message.key.remoteJid.includes("newsletter")) {
                logger_1.default.info(`[newsletter] Ignorando mensagem de newsletter no listener: ${message.key.remoteJid}`);
                return;
            }
        ...
        contacts.forEach(async (contact) => {
            ...
            if (typeof contact.imgUrl !== "undefined") {
                ...
                if (contact.remoteJidAlt) {
                    ...
                    logger_1.default.info(`✅ Usando remoteJidAlt: ${contactRemoteJid}`);
                }
                else if (contact.id.includes("@lid")) {
                    logger_1.default.warn(`⚠️ Contato com LID detectado: ${contact.id}`);
                    ...
                    if (existingContact && existingContact.number && existingContact.number.length <= 15) {
                        ...
                        logger_1.default.info(`✅ Número recuperado do banco: ${contactNumber}`);
                    }
                    else {
                        // ❌ Não conseguiu encontrar número real, IGNORAR atualização
                        logger_1.default.error(`❌ Não foi possível encontrar número real para LID: ${contact.id}`);
                ...
                const updatedContact = await (0, CreateOrUpdateContactService_1.default)(contactData);
                // 📱 Verificar se deve salvar no celular após atualizar contato
                if (!contactData.isGroup && updatedContact) {
                    try {
                        const shouldSave = await (0, ContactPhoneService_1.ShouldSaveToPhone)({
                            contact: updatedContact,
                            messageBody: "",
                            companyId
                        });
                        if (shouldSave && !updatedContact.savedToPhone) {
                            logger_1.default.info(`📱 Salvando contato ${updatedContact.id} no celular automaticamente`);
                            await (0, ContactPhoneService_1.SaveContactToPhone)({
                                contact: updatedContact,
                                whatsappId: wbot.id,
                                companyId
                            });
                        ...
                        logger_1.default.warn(`Erro ao verificar salvamento no celular: ${saveError.message}`);
                    }
                }
            }
        });
