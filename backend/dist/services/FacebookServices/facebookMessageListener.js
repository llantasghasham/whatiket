...
const verifyMessageFace = async (msg, body, ticket, contact, fromMe = false) => {
    const safeMsg = msg || {};
    const quotedMsg = await (0, exports.verifyQuotedMessage)(safeMsg);
    const wid = safeMsg.mid || safeMsg.message_id || `${ticket.id}-${Date.now()}`;
    const isEcho = safeMsg.is_echo ?? false;
    const textBody = safeMsg.text || body;
    const messageData = {
        wid,
        ticketId: ticket.id,
...
const verifyMessageMedia = async (msg, ticket, contact, fromMe = false) => {
    ...
    const messageData = {
        ...
        fromMe: fromMe ? fromMe : isEcho ? true : false,
        mediaType: safeMsg.attachments?.[0]?.type,
        mediaUrl: fileName,
        read: fromMe ? fromMe : isEcho,
        quotedMsgId: null,
        ack: 3,
            ...
            try {
                if (!fromMe) {
                    ...
                    if (ticket.status === "nps" &&
                        ticketTraking !== null &&
                        (0, wbotMessageListener_1.verifyRating)(ticketTraking)) {
                    ...
                    if (enableLGPD && ticket.status === "lgpd") {
                        if ((0, lodash_1.isNil)(ticket.lgpdAcceptedAt) &&
                            ...
                            if (!Number.isNaN(choosenOption) &&
                                ...
                                if (choosenOption === 1) {
                                    ...
                                    if (getSession.complationMessage !== "" &&
                        ...
                        if (!(0, lodash_1.isNil)(ticket.lgpdSendMessageAt) &&
                            (0, lodash_1.isNil)(ticket.lgpdAcceptedAt))
                            return;
                    }
                }
            }
