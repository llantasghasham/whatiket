...
const WhitelabelService_1 = require("../SettingService/WhitelabelService");
...
const getAccessToken = async (companyId) => {
    let facebookAppId = process.env.FACEBOOK_APP_ID || "2813216208828642";
    let facebookAppSecret = process.env.FACEBOOK_APP_SECRET || "8233912aeade366dd8e2ebef6be256b6";
    if (companyId) {
...
const sendText = async (id, text, token) => {
    ...
    const url = `${(0, graphApiHelper_1.getGraphBaseUrl)()}/me/messages`;
...
const getAccessTokenFromPage = async (token, companyId) => {
    try {
        if (!token)
