...
const CalculatePotentialScore = (messageBody) => {
    try {
        let score = 0;
...
const UpdateContactScore = async ({ contactId, messageBody, companyId }) => {
    try {
        const score = (0, exports.CalculatePotentialScore)(messageBody);
        const [updatedRowsCount] = await Contact_1.default.update({
        ...
        if (updatedRowsCount > 0) {
            logger_1.default.info(`Contact ${contactId} score updated: ${score} (isPotential: ${score >= 5})`);
        }
        else {
            logger_1.default.warn(`Contact ${contactId} not found for score update`);
        }
    }
    catch (error) {
        logger_1.default.error("Error updating contact score:", error);
        throw error;
    }
};
exports.UpdateContactScore = UpdateContactScore;
const GetContactStats = async (companyId) => {
    try {
        ...
        const lidStabilityDistribution = await Contact_1.default.findAll({
            where: { companyId },
            attributes: [
                'lidStability',
                [sequelize_2.Sequelize.fn('COUNT', sequelize_2.Sequelize.col('id')), 'count']
...
const GetHighPotentialContacts = async (companyId, limit = 50) => {
    try {
        const contacts = await Contact_1.default.findAll({
            where: {
                companyId,
                isPotential: true,
                potentialScore: { [sequelize_1.Op.gte]: 7 }
            },
        ...
        logger_1.default.error("Error getting high potential contacts:", error);
        throw error;
    }
};
exports.GetHighPotentialContacts = GetHighPotentialContacts;
const BatchUpdateContactScores = async (contacts) => {
    try {
        ...
        logger_1.default.info(`Batch updated ${contacts.length} contact scores`);
    }
    catch (error) {
        logger_1.default.error("Error in batch contact score update:", error);
        throw error;
    }
};
