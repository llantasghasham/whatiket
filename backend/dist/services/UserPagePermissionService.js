...
class UserPagePermissionService {
    ...
    static async getUserPagePermissions(userId) {
    ...
    static async setUserPagePermissions(data) {
        ...
        if (pagePermissionsMode === "custom") {
            ...
            await UserPagePermission_1.default.destroy({ where: { userId } });
            ...
            await UserPagePermission_1.default.bulkCreate(permissionsToCreate);
        }
        else {
    ...
    static async canUserAccessPage(userId, pagePath) {
        const user = await User_1.default.findByPk(userId, {
            include: [{ model: UserPagePermission_1.default, as: "pagePermissions" }]
        });
        if (!user) {
    ...
    static async getUserAccessiblePages(userId) {
