...
const SendMessageFlow_1 = require("../../helpers/SendMessageFlow");
...
const FollowUpNodeService_1 = __importDefault(require("../FlowBuilderService/FollowUpNodeService"));
...
const pdfUtils_1 = require("../../helpers/pdfUtils");
const directOpenAiMessageBuffer = {};
const ActionsWebhookService = async (whatsappId, idFlowDb, companyId, nodes, connects, nextStage, dataWebhook, details, hashWebhookId, pressKey, idTicket, numberPhrase = "", msg) => {
    try {
        ...
        if (String(next) === '1') {
            ...
            if (idTicket) {
                const activeTicket = await Ticket_1.default.findOne({
                    where: { id: idTicket, flowWebhook: true, lastFlowId: { [sequelize_1.Op.ne]: null } }
                });
        ...
        const checkAndCloseTicketIfAssigned = async (ticketId) => {
            try {
                ...
                if (ticket && ticket.userId) {
                    ...
                    const io = (0, socket_1.getIO)();
                    const ticketUpdated = await (0, ShowTicketService_1.default)(ticket.id, companyId);
        ...
        for (var i = 0; i < lengthLoop; i++) {
            ...
            const awaitingAsaasResponse = !!pressKey &&
            ...
            if (pressKey) {
                ...
                if (otherNode) {
                    ...
                    console.log("Nós disponíveis:", nodes.map(n => ({ id: n.id, type: n.type })));
                }
            }
            ...
            if (nodeSelected.type === "menu") {
                ...
                if (next && next !== nodeSelected.id) {
                    ...
                    if (nextNode) {
                        ...
                        return await (0, exports.ActionsWebhookService)(whatsappId, idFlowDb, companyId, nodes, connects, next, dataWebhook, details, hashWebhookId, pressKey, idTicket, numberPhrase, msg);
                    }
                }
            }
                    ...
                    if (msg) {
                        ...
                        directOpenAiMessageBuffer[ticket.id].timeout = setTimeout(async () => {
                            try {
                                ...
                                await (0, OpenAiService_1.handleOpenAi)(buffer.openAiSettings, combinedMsg, buffer.wbot, buffer.ticket, buffer.contact, null, buffer.ticketTraking);
                                console.log(`DirectOpenAI: Processamento concluído com sucesso`);
                            }
                            catch (error) {
            ...
            if (nodeSelected.type === "followUp") {
                ...
                if (!ticket && idTicket) {
                    ticket = await Ticket_1.default.findOne({ where: { id: idTicket, companyId } });
                }
                if (!ticket) {
                ...
                try {
                    await FollowUpNodeService_1.default.executeFollowUpNode(nodeSelected.data, ticket.id, companyId, nodeSelected.id);
                    console.log(`FollowUp: agendado com delay ${nodeSelected?.data?.delayMinutes} min para ticket ${ticket.id}`);
                }
                catch (error) {
            ...
            if (nodeSelected.type === "question") {
                ...
                const answerKey = questionData?.answerKey || `question_${nodeSelected.id}`;
                const currentWebhookData = ticket?.dataWebhook || {};
                const questionState = currentWebhookData?.questionState;
                const awaitingAnswer = questionState?.awaiting === true &&
                ...
                const questionMessage = (0, Mustache_1.default)(`${message || ""}`, ticket.contact);
                ...
                await (0, bluebird_1.delay)(2000);
                await (0, SendWhatsAppMediaFlow_1.typeSimulation)(ticket, "paused");
                ...
                const targetQueueId = nodeSelected?.data?.queueId ??
                    nodeSelected?.data?.typebotIntegration?.queueId ??
                    ticket.queueId;
                if (!targetQueueId) {
                    console.warn("ActionsWebhookService question node sem queueId definido; pulando atualização de fila.");
                    break;
                }
                const queue = await (0, ShowQueueService_1.default)(Number(targetQueueId), companyId);
                await ticket.update({
                    status: "pending",
                    queueId: queue.id,
                ...
                if (enableQueuePosition) {
                    ...
                    const ticketFilaDetails = await (0, ShowTicketService_1.default)(ticket.id, companyId);
                ...
                for (var iLoc = 0; iLoc < nodeSelected.data.seq.length; iLoc++) {
                    ...
                    if (elementNowSelected.includes("img")) {
                        ...
                        await (0, SendMessage_1.SendMessage)(whatsapp, {
                            number: numberClient,
                            body: "",
                            mediaPath: (await (0, UrlService_1.getBackendUrl)(companyId)) === "https://localhost:8090"
                    ...
                    if (elementNowSelected.includes("audio")) {
                        const mediaDirectory = (await (0, UrlService_1.getBackendUrl)(companyId)) === "https://localhost:8090"
                            ? `${__dirname.split("src")[0].split("\\").join("/")}public/${nodeSelected.data.elements.filter(item => item.number === elementNowSelected)[0].value}`
                    ...
                    if (elementNowSelected.includes("file") ||
                        ...
                        const fileValue = nodeSelected.data.elements.filter(item => item.number === elementNowSelected)[0].value;
                        const mediaDirectory = path_1.default.join(path_1.default.resolve(__dirname, "../../.."), "public", "uploads", fileValue);
                        ...
                        const mimeType = mime_types_1.default.lookup(mediaDirectory) || "application/octet-stream";
            ...
            if (nodeSelected.type === "file") {
                ...
                const mediaDirectory = path_1.default.join(path_1.default.resolve(__dirname, "../../.."), "public", "uploads", nodeSelected.data.url);
                const contact = await Contact_1.default.findOne({
                    where: { number: numberClient, companyId }
                });
                if (!fs_1.default.existsSync(mediaDirectory)) {
            ...
            if (nodeSelected.type === "transferFlow") {
                ...
                if (targetFlowId) {
                    ...
                    const targetFlow = await FlowBuilder_1.FlowBuilderModel.findOne({
                    ...
                    if (targetFlow && targetFlow.flow) {
                        ...
                        if (startNode) {
                            ...
                            if (startConnection) {
                                ...
                                const newHashFlowId = (0, randomCode_1.randomString)(42);
                                ...
                                return await (0, exports.ActionsWebhookService)(whatsappId, targetFlowId, companyId, newNodes, newConnects, startConnection.target, dataWebhook, details, newHashFlowId, undefined, idTicket, numberPhrase, msg);
                            }
                        }
                    }
                    else {
                ...
                if (tagId && ticket) {
                    ...
                    try {
                        ...
                        if (tag) {
                            ...
                            const existingTag = await TicketTag_1.default.findOne({
                            ...
                            if (!existingTag) {
                                ...
                                try {
                                    ...
                                    const newContactTag = await ContactTag_1.default.create({
                                    ...
                                    const newTicketTag = await TicketTag_1.default.create({
                                        ticketId: ticket.id,
                                        tagId: tag.id
                                    });
                ...
                if (tagId && ticket) {
                    ...
                    try {
                        ...
                        if (tag) {
                            ...
                            const existingKanbanTags = await TicketTag_1.default.findAll({
                ...
                if (queueId && ticket) {
                    ...
                    try {
                        ...
                        if (queue) {
                            ...
                            const ticketAfterUpdate = await (0, ShowTicketService_1.default)(ticket.id, companyId);
                ...
                if (tagId && ticket) {
                    ...
                    try {
                        ...
                        if (tag) {
                            ...
                            await ContactTag_1.default.create({
                                contactId: ticket.contactId,
                                tagId: tagId
                            });
            ...
            const normalizeVariableKey = (rawValue) => {
                if (!rawValue)
                    return "";
            ...
            if (nodeSelected.type === "asaas") {
                ...
                const awaitingCpf = asaasState?.awaiting === true &&
                    asaasState?.nodeId === nodeSelected.id;
                if (!awaitingCpf) {
                    // Enviar mensagem solicitando CPF e marcar estado de espera
                    await (0, SendMessageFlow_1.SendMessageFlow)(whatsapp, {
                        number: numberClient,
                        body: message
                    }, ticket?.id, ticket);
                    if (ticket) {
                ...
                if (pressKey && pressKey !== "999") {
                    ...
                    const resultConnect = connects.filter(connect => connect.source === nodeSelected.id);
                    try {
                        const boletoData = await (0, PaymentGatewayService_1.getAsaasSecondCopyByCpf)(companyId, cpf);
                        ...
                        if (boletoFileSource) {
                            try {
                                ...
                                const { buffer: pdfBuffer, contentType } = await (0, pdfUtils_1.fetchPdfBufferFromUrl)(boletoFileSource);
            ...
            if (nodeSelected.type === "closeTicket") {
                ...
                if (nodeSelected.type === "googleSheets") {
                    ...
                    // Garantir que o ticket existe
                    if (!ticket && idTicket) {
                        ticket = await (0, ShowTicketService_1.default)(idTicket, companyId);
                    }
                    if (!ticket) {
                    ...
                    // Obter conexões para caminhos de erro
                    const resultConnect = connects.filter((connect) => connect.source === nodeSelected.id && connect.sourceHandle === "error");
                    // Extrair variáveis do dataWebhook
                    const variables = ticket?.dataWebhook?.variables || {};
                    ...
                    try {
                        ...
                        switch (operation) {
                            ...
                            case "add":
                                ...
                                break;
                            case "edit":
                                result = await sheetsService.editRow(sheetsConfig, nodeSelected.data?.searchColumn || "", nodeSelected.data?.searchValue || "", nodeSelected.data?.rowData || {}, variables);
                                break;
                            case "delete":
                                result = await sheetsService.deleteRow(sheetsConfig, nodeSelected.data?.searchColumn || "", nodeSelected.data?.searchValue || "", variables);
                                break;
                            case "search":
                                result = await sheetsService.searchData(sheetsConfig, nodeSelected.data?.searchColumn || "", nodeSelected.data?.searchValue || "", variables);
                                break;
                            default:
                        ...
                        if (resultConnect.length > 0) {
                            const nextNode = nodes.find((node) => node.id === resultConnect[0].target);
                            if (nextNode) {
                                await (0, exports.ActionsWebhookService)(whatsappId, idFlowDb, companyId, nodes, connects, nextNode.id, dataWebhook, details, hashWebhookId, pressKey, idTicket, numberPhrase, msg);
                            }
                        }
                        break;
                    }
                ...
                if (ticket) {
                    ...
                    if (message) {
                        ...
                        await (0, CreateMessageService_1.default)({ messageData: messageData, companyId });
                    }
                    // Encerrar o ticket
            ...
            if (nodeSelected.type === "menu") {
                ...
                if (pressKey) {
                    ...
                    const settingsMenu = await CompaniesSettings_1.default.findOne({
                        where: {
                            companyId: companyId
                        }
                    });
                    ...
                    if (typeBotMenu === "list") {
                        ...
                        const listPayload = {
                            text: (0, Mustache_1.default)(nodeSelected.data.message, ticket.contact),
                            buttonText: "Escolher opção",
                            sections
                        };
                        await wbot.sendMessage(`${numberClient}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, listPayload);
                    }
                    else {
                        await (0, SendWhatsAppMessage_1.default)({
                            body: msg.body,
            ...
            const hasNextNodes = connects.filter(connect => connect.source === nodeSelected.id).length >
                0;
