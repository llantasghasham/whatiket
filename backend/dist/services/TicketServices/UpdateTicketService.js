...
const UpdateTicketService = async ({ ticketData, ticketId, companyId }) => {
    try {
        let { queueId, userId, sendFarewellMessage = true, amountUsedBotQueues, lastMessage, integrationId, useIntegration, unreadMessages, leadValue, msgTransfer, isTransfered = false, status } = ticketData;
        ...
        const shouldClearAssignments = status === "closed" &&
            ...
            if (settings.userRating === "enabled" &&
                ...
                if (ticketTraking.ratingAt == null) {
                    ...
                    if (ticket.channel === "whatsapp" &&
                        ...
                        const msg = await (0, sendFacebookMessage_1.default)({
                            body: bodyRatingMessage,
                            ticket
                        });
        ...
        if (isTransfered) {
            if (settings.closeTicketOnTransfer) {
                ...
                await newTicketTransfer.update(newTicketUpdateData);
        ...
        const isClosingTicketFinal = status === "closed";
        const nextQueueId = isClosingTicketFinal
            ? null
            : queueId !== undefined
                ? queueId
                : ticket.queueId;
        const nextUserId = isClosingTicketFinal
            ? null
            : userId !== undefined
                ? userId
                : ticket.userId;
        // Verifica se é apenas mudança de fila/usuário (sem mudança de status ou mensagem)
        const isOnlyQueueOrUserChange = status === ticket.status &&
            !lastMessage &&
        ...
        if (ticket.status !== oldStatus ||
            ...
            console.log(`Emitindo delete para ticket ${ticket.id} - Status mudou de ${oldStatus} para ${ticket.status}`);
        ...
        console.log(`Emitindo update para ticket ${ticket.id} - Status: ${ticket.status}, User: ${ticket.userId}`);
        ...
        if (oldStatus === "pending" && status === "open") {
            console.log(`Ticket ${ticket.id} aceito - emitindo evento de aceitação`);
            io.of(String(companyId))
                .emit(`company-${companyId}-ticket`, {
                action: "accept",
                ticketId: ticket.id,
