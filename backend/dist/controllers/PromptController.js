...
const store = async (req, res) => {
    ...
    const decoded = (0, jsonwebtoken_1.verify)(token, auth_1.default.secret);
    const { companyId } = decoded;
    const { name, apiKey, prompt, maxTokens, temperature, promptTokens, completionTokens, totalTokens, queueId, maxMessages, voice, voiceKey, voiceRegion, model, toolsEnabled, knowledgeBase } = req.body;
    console.log("[PromptController.store] toolsEnabled:", toolsEnabled);
    const promptTable = await (0, CreatePromptService_1.default)({
        name,
        apiKey,
        prompt,
        maxTokens,
