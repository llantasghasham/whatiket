...
const show = async (req, res) => {
    ...
    const { companyId } = req.user;
    const tag = await (0, ShowService_1.default)(tagId, companyId);
    return res.status(200).json(tag);
};
exports.show = show;
const update = async (req, res) => {
    ...
    const { companyId } = req.user;
    const tag = await (0, UpdateService_1.default)({ tagData, id: tagId, companyId });
    const io = (0, socket_1.getIO)();
    io.of(String(companyId))
        .emit(`company${companyId}-tag`, {
        action: "update",
...
const remove = async (req, res) => {
    const { tagId } = req.params;
    const { companyId } = req.user;
    await (0, DeleteService_1.default)({ id: tagId, companyId });
    const io = (0, socket_1.getIO)();
    io.of(String(companyId))
        .emit(`company${companyId}-tag`, {
        action: "delete",
...
const kanban = async (req, res) => {
    ...
    const negocio = await Negocio_1.default.findOne({
        where: { id: negocioId, companyId }
    });
    if (!negocio || !negocio.kanbanBoards) {
        return res.json({ lista: [] });
    }
    const allowedIds = Array.isArray(negocio.kanbanBoards)
        ? negocio.kanbanBoards
        : [];
    const filtered = tags.filter(tag => allowedIds.includes(tag.id));
    return res.json({ lista: filtered });
};
