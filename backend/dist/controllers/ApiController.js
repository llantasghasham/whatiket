...
const index = async (req, res) => {
    ...
    const { number, body, quotedMsg, userId, queueId, sendSignature = false, closeTicket = false, noRegister = true } = req.body;
    ...
    const convertBinToPdfIfNeeded = (media) => {
        const publicFolder = path_1.default.resolve(__dirname, "..", "..", "public");
        const originalPath = path_1.default.join(publicFolder, `company${companyId}`, media.filename);
        const ext = path_1.default.extname(media.originalname || media.filename || "").toLowerCase();
        const isBinExt = ext === ".bin";
        const isBinMime = media.mimetype === "application/octet-stream";
        if (isBinExt || isBinMime) {
            const baseName = path_1.default.basename(media.filename, path_1.default.extname(media.filename));
            const newFilename = `${baseName}.pdf`;
            const newPath = path_1.default.join(publicFolder, `company${companyId}`, newFilename);
            try {
            ...
            return { media, filePath: newPath };
        }
        return { media, filePath: originalPath };
    };
    let user;
    ...
    if (noRegister) {
        if (medias) {
            try {
                ...
                await Promise.all(medias.map(async (media) => {
                    const { filePath, media: adjustedMedia } = convertBinToPdfIfNeeded(media);
                    const options = await (0, SendWhatsAppMedia_1.getMessageOptions)(adjustedMedia.filename, filePath, companyId.toString(), `\u200e ${bodyMessage}`);
        ...
        if (medias) {
            try {
                await Promise.all(medias.map(async (media) => {
                    const { media: adjustedMedia } = convertBinToPdfIfNeeded(media);
                    sentMessage = await (0, SendWhatsAppMedia_1.default)({
                        body: `\u200e ${bodyMessage}`,
                        media: adjustedMedia,
                    ...
                    const publicFolder = path_1.default.resolve(__dirname, "..", "..", "public");
                    const filePath = path_1.default.join(publicFolder, `company${companyId}`, adjustedMedia.filename);
                    const fileExists = fs_1.default.existsSync(filePath);
                    if (fileExists) {
                        fs_1.default.unlinkSync(filePath);
                    }
