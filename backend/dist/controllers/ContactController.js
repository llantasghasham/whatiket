...
const deleteContactFile = async (req, res) => {
    ...
    const { companyId } = req.user;
    const contact = await Contact_1.default.findOne({ where: { id: contactId, companyId } });
    if (!contact) {
        throw new AppError_1.default("ERR_NO_CONTACT_FOUND", 404);
    }
    const existingFiles = (contact.files || []);
    ...
    try {
        ...
        const filePath = path_1.default.resolve(publicFolder, `company${companyId}`, "contacts", String(contactId), filename);
...
const uploadContactFiles = async (req, res) => {
    const { contactId } = req.params;
    const { companyId } = req.user;
    const files = req.files;
    const contact = await Contact_1.default.findOne({ where: { id: contactId, companyId } });
