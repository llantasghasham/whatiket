...
exports.storePrivateFile = exports.certUpload = exports.deleteDashboardImage = exports.storeLogo = exports.publicShow = exports.updateOne = exports.getSetting = exports.update = exports.showOne = exports.index = void 0;
...
const storeLogo = async (req, res) => {
    const file = req.file;
    const { mode, typeArch } = req.body;
    const { companyId } = req.user;
    const validModes = ["Light", "Dark", "Favicon", "Loading", "Image", "image-1", "image-2", "image-3"];
    ...
    if (file && file.mimetype.startsWith("image/")) {
        ...
        if (typeArch === "dashboard") {
            settingKey = `dashboardImage${mode.replace("image-", "")}`;
        }
...
const deleteDashboardImage = async (req, res) => {
    ...
    const { companyId } = req.user;
    try {
        // Find and delete setting from database
        const Setting = require("../models/Setting").default;
        await Setting.destroy({
            where: {
                key: `dashboardImage${index}`,
                companyId
            }
        });
        ...
        const filePath = path.join(__dirname, '..', '..', '..', 'public', `dashboard-image-${index}.png`);
