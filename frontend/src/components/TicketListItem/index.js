...
const getChannelColor = (channel) => {
    if (channel === "facebook") {
        return "#1877F2"; // Azul do Facebook
    }
    if (channel === "instagram") {
        ...
        <React.Fragment key={ticket.id}>
            ...
            >
                    ...
                    <Badge
                        ...
                        badgeContent={
                            <SmallAvatar
                                ...
                                style={{ 
                                    backgroundColor: getChannelColor(ticket?.channel),
                                    border: `2px solid ${getChannelColor(ticket?.channel)}`
                                }}
                            />
                        }
                    >
                ...
                <div className={classes.divTags}>
                    ...
                    {(() => {
                        const contactTags = (ticket.contact && ticket.contact.tags) ? ticket.contact.tags : [];
                        const kanbanTags = ticket.tags || [];

                        // Unifica tags de contato e tags Kanban, removendo duplicadas por id
                        const allTags = [...contactTags, ...kanbanTags].filter((tag, index, arr) =>
                            tag && index === arr.findIndex(t => t && t.id === tag.id)
                        );

                        if (!allTags.length || !allTags[0]?.name) return null;

                        return (
                            <>
                                <div className={classes.tagsWrapper}>
                                    <div
                                        key={allTags[0].id || "tag-0"}
                                        className={classes.tags}
                                        title={allTags[0].name || "Tag"}
                                        style={{
                                            backgroundColor: allTags[0].color || "#999999",
                                        }}
                                    >
                                        {allTags[0].name || "Tag"}
                                    </div>
                                </div>
                                {allTags.length > 1 && allTags[1]?.name && (
                                    <div
                                        key={allTags[1].id || "tag-1"}
                                        className={classes.tags}
                                        title={allTags[1].name || "Tag"}
                                        style={{
                                            backgroundColor: allTags[1].color || "#999999",
                                        }}
                                    >
                                        +{allTags.length - 1}
                                    </div>
