...
const CampaignModalPhrase = ({ open, onClose, FlowCampaignId, onSave }) => {
  ...
  const applicationSaveAndEdit = () => {
    ...
    if (FlowCampaignId) {
      ...
      onClose();
      onSave('ok');
      toast.success("Frase modificada con éxito!");
      clearData();
    } else {
      ...
      >
        <DialogTitle id="form-dialog-title" className={classes.dialogTitle}>
          {campaignEditable ? (
            <>
              {FlowCampaignId
                ? `Editar campaña con flujo por frase`
                : `Nueva campaña con flujo por frase`}
            </>
          ) : (
            <>Solo lectura</>
          )}
        ...
        {!loading && (
          <Stack sx={{ padding: "52px" }}>
            <Stack sx={{ gap: "14px" }}>
              <Stack gap={1}>
                <Typography>Nombre del disparo por frase</Typography>
                <TextField
                  label={""}
                  name="text"
                  variant="outlined"
              ...
              <Stack gap={1}>
                <Typography>Elige un flujo</Typography>
                <Autocomplete
                  disablePortal
                  id="combo-box-demo"
                  value={flowSelected}
              ...
              <Stack gap={1}>
                <Typography>¿Qué frase activa el flujo?</Typography>
                <TextField
                  label={""}
                  name="text"
                  variant="outlined"
