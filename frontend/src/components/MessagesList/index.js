...
const checkMessageMedia = (message) => {
  ...
  // Tratar mensagens de visualização única (viewOnce) como mídia normal
  else if (message.mediaType === "viewOnceMessageV2" && message.mediaUrl) {
    // Detectar o tipo de mídia pela URL ou mimetype
    const mediaUrl = message.mediaUrl.toLowerCase();
    if (mediaUrl.includes('.jpg') || mediaUrl.includes('.jpeg') || mediaUrl.includes('.png') || mediaUrl.includes('.gif') || mediaUrl.includes('.webp') || mediaUrl.includes('image')) {
      return <ModalImageCors imageUrl={message.mediaUrl} />;
    } else if (mediaUrl.includes('.mp4') || mediaUrl.includes('.mov') || mediaUrl.includes('.avi') || mediaUrl.includes('video')) {
      return (
        <video
    ...
    } else if (mediaUrl.includes('.ogg') || mediaUrl.includes('.mp3') || mediaUrl.includes('.opus') || mediaUrl.includes('audio') || mediaUrl.includes('ptt')) {
...
const buildReactionDescription = (message, isFromMe) => {
  ...
  if (message?.quotedMsg) {
    const reactedTarget = message.quotedMsg.fromMe
      ? " sua mensagem"
      : message.quotedMsg?.contact?.name
        ? ` a mensagem de ${message.quotedMsg.contact.name}`
        : " a uma mensagem";
    return `${actorName} reagiu${emojiPart}${reactedTarget}`;
  }

  return `${actorName} reagiu${emojiPart}`;
};
...
const renderQuotedMessage = (message) => {
  ...
  const renderQuotedContent = () => {
    if (AUDIO_MEDIA_TYPES.has(quoted.mediaType)) {
      if (!quoted.mediaUrl) {
        return <span>Áudio indisponível</span>;
      }

      return (
        <div className={classes.downloadMedia}>
          <AudioModal url={quoted.mediaUrl} />
        </div>
      );
    }

    if (quoted.mediaType === "video" && quoted.mediaUrl) {
      return (
        <video className={classes.messageMedia} src={quoted.mediaUrl} controls />
      );
    }

    if (quoted.mediaType === "image" && quoted.mediaUrl) {
      return <ModalImageCors imageUrl={quoted.mediaUrl} />;
    }

    if (quoted.mediaType === "sticker" && quoted.mediaUrl) {
      return <img src={quoted.mediaUrl} alt="Sticker" className={classes.messageMedia} />;
    }
  ...
  return (
    ...
    >
      ...
      <div className={classes.quotedMsg}>
        {!quoted?.fromMe && quoted?.contact?.name && (
          <span className={classes.messageContactName}>{quoted.contact.name}</span>
        )}
        {renderQuotedContent()}
      </div>
    </div>
                ...
                {message.mediaType === "reactionMessage" && (
                  <>
                    <span style={{ marginLeft: "0px" }}>
                      <MarkdownWrapper>
                        {buildReactionDescription(message, false)}
                      </MarkdownWrapper>
                    </span>
                  </>
                )}
              ...
              >
                ...
                {message.mediaType === "reactionMessage" && (
                  <>
                    <span style={{ marginLeft: "0px" }}>
                      <MarkdownWrapper>
                        {buildReactionDescription(message, true)}
                      </MarkdownWrapper>
                    </span>
                  </>
                )}
                ...
                <span className={classes.timestamp}>
                  {user?.name && (
                    <span className={classes.senderName}>
                      {user.name} • {format(parseISO(message.createdAt), "dd/MM/yyyy")} •{" "}
                    </span>
                  )}
                  {message.isEdited ? "Editada " + format(parseISO(message.createdAt), "HH:mm") : format(parseISO(message.createdAt), "HH:mm")}
