...
const useStyles = makeStyles((theme) => ({
  ...
  messagesList: {
    ...
    "&::-webkit-scrollbar-thumb": {
      background: theme.mode === 'light' ? "rgba(0,0,0,0.3)" : "rgba(255,255,255,0.3)",
      borderRadius: "4px",
      "&:hover": {
        background: theme.mode === 'light' ? "rgba(0,0,0,0.5)" : "rgba(255,255,255,0.5)",
      },
    },
    
    // Fallback para Firefox
    scrollbarWidth: "thin",
    scrollbarColor: theme.mode === 'light' ? "rgba(0,0,0,0.3) rgba(0,0,0,0.1)" : "rgba(255,255,255,0.3) rgba(255,255,255,0.1)",
...
// Función adicional para scroll manual hacia arriba
const scrollToTop = () => {
...
const handleScroll = (e) => {
  ...
  // Mantener una pequeña distancia del top para evitar problemas
  if (scrollTop === 0) {
  ...
  // Cargar más mensajes cuando se está cerca del top (primeros 100px)
  if (scrollTop < 100) {
    loadMore();
  }

  // Opcional: Auto-scroll hacia abajo cuando hay nuevos mensajes y el usuario está cerca del final
  const isNearBottom = scrollHeight - scrollTop - clientHeight < 100;
  if (isNearBottom && messagesList.length > 0) {
    // El usuario está cerca del final, mantener scroll automático para nuevos mensajes
    setTimeout(() => {
      if (lastMessageRef.current) {
...
const checkMessageMedia = (message) => {
  ...
  try {
      ...
      if (!nativeFlowMessage) {
          console.error("Mensaje de flujo nativo no encontrado.");
          return; // Sale de la función si no encuentra el flujo nativo
      }
      const button = nativeFlowMessage.buttons?.[0]; // Considera que hay un único botón
      if (!button) {
